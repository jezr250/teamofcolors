...
# 並び順もそのまま一覧の表示順になる（モルタル→内装→エイジング→特殊塗装→氷壁）。
CATEGORIES = [
    ("モルタル造形", "mortar", "モルタル造形制作"),
    ("内装・インテリア塗装", "interior", "内装・インテリア塗装"),
    ("エイジング塗装", "aging", "エイジング塗装"),
    # 表示名は 2026-09-17 の修正依頼 02 で「特殊塗装・特殊左官」に変更（元フォルダ名はそのまま）
    ("特殊塗装", "special", "特殊塗装・特殊左官"),
    # 「氷壁」は商標登録出願中。その旨の表示はサイト側（サービスタイル）で行うので、
    # ここでは他と同じ扱いにする。元フォルダ名の「【商標登録出願中】」は落としてある。
    ("氷壁", "hyoheki", "氷壁"),
]
